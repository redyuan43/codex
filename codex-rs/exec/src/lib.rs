    ...
    let model_provider = if oss {
        ...
        if let Some(provider) = resolved {
            ...
            return Err(anyhow::anyhow!(
                "No default OSS provider configured. Use --local-provider=provider or set oss_provider to one of: {LLAMACPP_OSS_PROVIDER_ID}, {LMSTUDIO_OSS_PROVIDER_ID}, {OLLAMA_OSS_PROVIDER_ID} in config.toml"
            ));
        }
    } else {
