...
impl ChatWidget {
    ...
    pub(super) fn on_hook_completed(
        ...
        let has_summary_source = completed
            .summary_input
            .as_deref()
            .is_some_and(|value| !value.trim().is_empty())
            || !completed.entries.is_empty()
            || completed.status != codex_app_server_protocol::HookRunStatus::Completed;
        ...
        if let Some(summary) = summary {
            self.set_hook_placeholder_summary(Some(format!("摘要：{summary}")));
        }
        self.request_redraw();
    }
