        ...
        match cmd {
            ...
            SlashCommand::Loop => {
                ...
                if descriptions.is_empty() {
                    self.add_info_message(
                        "No active /loop schedules.".to_string(),
                        Some(
                            "Use /loop <duration> <prompt>, /loop every <duration> <prompt>, or /loop stop [id]."
                                .to_string(),
                        ),
                    );
                } else {
                    self.add_info_message(
                        format!("Active /loop schedules:\n{}", descriptions.join("\n")),
                        Some("Use /loop stop [id] to cancel.".to_string()),
                    );
                }
            }
    ...
    fn dispatch_prepared_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Loop if !trimmed.is_empty() => {
                ...
                if lower == "stop" {
                    let stopped = crate::loop_scheduler::stop_all_loops();
                    if stopped.is_empty() {
                        self.add_info_message(
                            "No active /loop schedules.".to_string(),
                            /*hint*/ None,
                        ...
                        let ids = stopped
                            ...
                            .join(", ");
                        self.add_info_message(
                            format!("Stopped /loop schedule(s): {ids}"),
                            /*hint*/ None,
                        );
                    }
                } else if let Some(rest) = lower.strip_prefix("stop ") {
                    match crate::loop_scheduler::parse_loop_id(rest) {
                        Ok(id) if crate::loop_scheduler::stop_loop(id) => {
                            self.add_info_message(
                                format!("Stopped /loop schedule #{id}."),
                                /*hint*/ None,
                            );
                        }
                        Ok(id) => {
                            self.add_error_message(format!("No active /loop schedule #{id}."));
                        }
