...
pub(crate) struct ChatWidget {
    ...
    review: ReviewState,
    // Last completed hook summary shown while the composer is idle.
    hook_placeholder_summary: Option<String>,
    // Best-effort one-line summary of the current execution stage.
    live_stage_summary: Option<String>,
    // Most recent completed step that helps rehydrate session progress.
...
impl ChatWidget {
    ...
    fn sync_runtime_placeholder_summary(&mut self) {
        let summary = match (
            &self.hook_placeholder_summary,
            &self.live_stage_summary,
            &self.recent_progress_summary,
        ) {
            (Some(hook), _, _) => Some(hook.clone()),
    ...
    fn set_hook_placeholder_summary(&mut self, summary: Option<String>) {
        let normalized = summary.and_then(|summary| normalize_summary_line(&summary));
        if self.hook_placeholder_summary == normalized {
            return;
        }
        self.hook_placeholder_summary = normalized;
    ...
    fn update_live_stage_summary_from_header(&mut self, header: &str) {
        let normalized = normalize_summary_line(header).filter(|header| header != "Working");
...
fn summarize_command_progress(command: &[String], parsed: &[ParsedCommand]) -> Option<String> {
    for parsed_cmd in parsed {
        if let ParsedCommand::Search {
            query: Some(query), ..
        } = parsed_cmd
        {
            let query = normalize_summary_line(query)?;
            return Some(format!("Searched for `{query}`"));
        }
    }
    summarize_unknown_command_progress(&command.join(" "))
}
...
fn summarize_unknown_command_progress(command: &str) -> Option<String> {
    ...
    (!first.is_empty()).then(|| format!("Finished `{first}`"))
}
