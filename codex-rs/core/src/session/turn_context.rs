...
pub(crate) struct NewTurnContextOptions {
    ...
    /// Overrides the selected model for this turn only, without persisting the change.
...
impl Session {
    ...
    pub(super) async fn new_turn_with_sub_id_if(
        ...
        // Overrides are scoped to this turn so the persisted thread settings keep the
        // originally selected model.
        if let Some(model_override) = options.model_override.as_deref() {
            let step_settings = Arc::make_mut(&mut configuration.step_settings);
