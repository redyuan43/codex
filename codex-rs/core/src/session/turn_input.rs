...
/// Siyuan model slugs used for the image fallback below.
const GPT_5_3_CODEX_SPARK: &str = "gpt-5.3-codex-spark";
const GPT_5_6_LUNA: &str = "gpt-5.6-luna";

/// Routes turns that include image input away from the spark model, which
/// cannot process images, to the luna model.
...
async fn start_or_steer(
    ...
    let model_override =
        image_fallback_model(session.collaboration_mode().await.model(), &items).map(String::from);
...
async fn start_if_idle(
    ...
    let model_override = match &input {
        SubmittedTurnInput::UserInput { content, .. } => {
            image_fallback_model(session.collaboration_mode().await.model(), content)
                .map(String::from)
        }
        _ => None,
    };
    let turn_context = match settings
        .apply_started(session, submission_id.clone(), kind, model_override)
        .await
    {
